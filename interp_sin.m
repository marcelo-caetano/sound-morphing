...
for iframe = 1:nframe
    ...
    for itrack = 1:ntrack(indtrack(2))
        ...
        if not(isnan(freq1(itrack,iframe))) && not(isnan(freq2(itrack,iframe)))
            ...
            interp_amp(itrack,iframe) = alpha*amp1(itrack,iframe) + (1-alpha)*amp2(itrack,iframe);
            ...
            interp_freq(itrack,iframe) = alpha*freq1(itrack,iframe)*2^((1-alpha)*log2(freq2(itrack,iframe)/freq1(itrack,iframe)))...
        ...
        elseif not(isnan(freq1(itrack,iframe))) && isnan(freq2(itrack,iframe))
            ...
            interp_amp(itrack,iframe) = alpha*amp1(itrack,iframe);
            % interp_freq(itrack,iframe) = alpha*freq1(itrack,iframe) + (1-alpha)*itrack*f02;
            % part_data{1}(i,2)*2^((1-beta)*log2(f02/f01))
            interp_freq(itrack,iframe) = freq1(itrack,iframe)*2^((alpha)*log2((itrack*f02)/freq1(itrack,iframe)));
            
            % Only 2 has harmonic
        elseif isnan(freq1(itrack,iframe)) && not(isnan(freq2(itrack,iframe)))
            
            interp_amp(itrack,iframe) = (1-alpha)*amp2(itrack,iframe);
            % interp_freq(itrack,iframe) = alpha*itrack*f01 + (1-alpha)*freq2(itrack,iframe);
            interp_freq(itrack,iframe) = freq2(itrack,iframe)*2^((1-alpha)*log2((itrack*f01)/freq2(itrack,iframe)));
