function [sinusoidal,partials,amplitudes,frequencies] = sinusoidal_resynthesis_PI(amp,freq,phase,delta,cframe,hopsize,winlen,sr,duration,cflag,maxnpeak)
...
for iframe = 1:nframe-1
    ...
    if iframe == 1 && cframe(iframe) > 1
        ...
        % [new_new_amp{iframe},new_new_amp{iframe+1},new_new_freq{iframe},new_new_freq{iframe+1},new_new_phase{iframe},new_new_phase{iframe+1}] = peak_matching_tracks(new_amp{iframe},new_amp{iframe+1},new_freq{iframe},new_freq{iframe+1},new_phase{iframe},new_phase{iframe+1},delta,hopsize,sr);
